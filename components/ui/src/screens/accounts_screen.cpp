...
const char* account_item_name(size_t index)
{
    static constexpr const char* EN[] = {"All", "Favorites", "Categories", "Search"};
    static constexpr const char* RU[] = {"Все", "Избранное", "Категории", "Поиск"};
    const bool russian = (i18n::language() == settings::Language::Russian);
    return russian ? RU[index] : EN[index];
}
...
void AccountsScreen::render()
{
    ...
    for (size_t i = 0; i < ITEM_COUNT; ++i) {
        ...
        lv_label_set_text_fmt(item_labels_[i], "%s%s", is_selected ? "> " : "", account_item_name(i));
    }
}
...
void AccountsScreen::activate()
{
    switch (static_cast<Item>(selected_)) {
        case Item::All: manager().push(std::make_unique<VaultListScreen>()); return;
        case Item::Favorites: manager().push(std::make_unique<FavoritesScreen>()); return;
        case Item::Categories: manager().push(std::make_unique<CategoriesScreen>()); return;
        case Item::Search: manager().push(std::make_unique<SearchScreen>()); return;
    }
}
...
{
    switch (action) {
        case InputAction::RotateLeft: move_selection(-1); return true;
        case InputAction::RotateRight: move_selection(+1); return true;
        case InputAction::OkShort: activate(); return true;
        case InputAction::BackShort: return false;
